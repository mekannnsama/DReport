...
namespace DReportUtil.Utils
{
    public class LogWriter
    {
        public static void _error(string TAG, string logtxt)
        {
            try
            {
                ...
                sw.WriteLine(string.Format(@"{0} [{1}] {2}", DateTime.Now.ToString(ConstantValues.FORMAT_DATE_TIME_MILISECONDS), TAG, logtxt));
                sw.Close();
                logtxt = string.Empty;
            }
            catch (Exception ex)
        ...
        public static void _report(string TAG, string logtxt)
        {
            try
            {
                var logfolder = AppDomain.CurrentDomain.BaseDirectory + "\\Log\\Report";
                System.IO.DirectoryInfo di = new System.IO.DirectoryInfo(logfolder);
                FileSystemAccessRule fsar = new FileSystemAccessRule("Users", FileSystemRights.FullControl, AccessControlType.Allow);
                DirectorySecurity ds = null;
                if (!di.Exists)
                ...
                ds = di.GetAccessControl();
                ds.AddAccessRule(fsar);
                StreamWriter sw = new StreamWriter(logfolder + "\\report" + DateTime.Today.ToString("yyyyMMdd") + ".txt", true);
                sw.WriteLine(string.Format(@"{0} [{1}] {2}", DateTime.Now.ToString(ConstantValues.FORMAT_DATE_TIME_MILISECONDS), TAG, logtxt));
                sw.Close();
                logtxt = string.Empty;
            }
    ...
    public class ConvertTool
    {
        ...
        public static string getDBRefNo(int len1, int len2)
        {
            string retStr = RandomString(len1) + RandomNum(len2);
            return retStr;
        }
    }
}
