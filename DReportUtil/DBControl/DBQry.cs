...
LEFT JOIN PRODUCT_CATALOG HH ON AA.PRODUCT_ID = HH.PRODUCT_ID
LEFT JOIN T_DISH_SUBSCRIBERTYPE SS ON BB.SUBSCRIBER_TYPE = SS.SUBTYPE_ID)) 
WHERE TRANS_ID NOT IN (SELECT TRANS_ID FROM FLASH_LEASING_CDR)", begin, end);
            return qry;
        }
...
and INSERT_DATE<=to_date('{1}','yyyymmddhh24miss') and type=1
    ...
    (
        ...
        SELECT CARD_NO, SUBSCRIBER_FNAME, SUBSCRIBER_LNAME, CERTIFICATE_NO, BRANCHNAME, CREATE_USER, CREATE_DATE, BUNDLE_NAME, TOTAL, DEPOSIT, DISCOUNT, LEASING FROM 
                            (
                            SELECT A.card_no, SUBSCRIBER_FNAME, SUBSCRIBER_LNAME, CERTIFICATE_NO, cast(D.BRANCH_NAME as nvarchar2(100)) AS BRANCHNAME, A.CREATED_USER AS CREATE_USER, A.CREATE_DATE , 'FLASH_LEASING_2020_VOD' AS BUNDLE_NAME, 80000 AS TOTAL, 20000 AS DEPOSIT, 0 AS DISCOUNT, 60000 AS LEASING FROM flash_leasing a
...
FROM(SELECT CARD_NO, FIRST_BALANCE,INCOME,AMOUNT, LAST_BALANCE,SUBSCRIBER_FNAME,SUBSCRIBER_LNAME, CERTIFICATE_NO, BRANCH_NAME, CREATE_DATE, UPDATED_DATE, CREATE_USER, LEASING_NAME 
FROM(SELECT AA.CARD_NO, FIRST_BALANCE,INCOME, AMOUNT,LAST_BALANCE, TDC.SUBSCRIBER_FNAME, TDC.SUBSCRIBER_LNAME,TDC.CERTIFICATE_NO,AAA.CREATE_DATE,AAA.UPDATED_DATE,AAA.CREATE_USER,AAA.LEASING_NAME,SBL.BRANCHNAME BRANCH_NAME 
FROM(SELECT CARD_NO, FB1 AS FIRST_BALANCE,INCOME,PP AS AMOUNT,FB1 + INCOME - PP AS LAST_BALANCE 
FROM(SELECT CARD_NO,FB - AAA FB1,INCOME,PP,LB,AAA 
FROM(SELECT A.CARD_NO, FB,INCOME,PP, LB, NVL(AAA, 0) AAA 
FROM(SELECT CARD_NO,FB,INCOME, NVL(PP, 0) AS PP, NVL(LB, 0) AS LB 
FROM(SELECT CARD_NO, FB, INCOME, PP, LB 
FROM(SELECT A.CARD_NO, FB,INCOME, PP, LB 
FROM(SELECT CARD_NO, SUM(DEVICE_PRICE) FB, 0 AS INCOME, DEVICE_PRICE 
FROM   UNI_DISH.D_PRODUCT_PLAN  WHERE CREATE_DATE <= TO_DATE('{0}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO, 0,DEVICE_PRICE) A 
LEFT JOIN (SELECT CARD_NO , SUM(DEVICE_PRICE) AS PP 
FROM   UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE <= TO_DATE('{0}','YYYYMMDDHH24MISS') AND UPDATED_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO) B ON A.CARD_NO = B.CARD_NO 
LEFT JOIN (SELECT CARD_NO, SUM(LB) AS LB 
FROM (SELECT CARD_NO,LB FROM   (SELECT CARD_NO,NVL(SUM( DEVICE_PRICE),0) AS LB 
FROM   UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE('{0}','YYYYMMDDHH24MISS') AND UPDATED_DATE NOT BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO) 
UNION ALL 
(SELECT CARD_NO,NVL(SUM(DEVICE_PRICE), 0) AS LB 
 FROM   UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE('{0}','YYYYMMDDHH24MISS') AND TRANSID IS NULL AND DEVICE_PRICE != 0 GROUP  BY CARD_NO)) GROUP  BY CARD_NO) C ON A.CARD_NO = C.CARD_NO) 
UNION ALL 
(SELECT A.CARD_NO, FB,INCOME, PP,LB 
FROM (SELECT CARD_NO,0 AS FB,SUM(DEVICE_PRICE) INCOME,PROMOTION_ID,CREATE_USER 
FROM  UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO, 0, CREATE_USER, PROMOTION_ID) A 
LEFT JOIN (SELECT CARD_NO, NVL(SUM(DEVICE_PRICE) , 0) AS PP,PROMOTION_ID, CREATE_USER 
FROM   UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE('{0}', 'YYYYMMDDHH24MISS') AND TO_DATE('{1}', 'YYYYMMDDHH24MISS') AND UPDATED_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO, PROMOTION_ID, CREATE_USER) B ON A.CARD_NO = B.CARD_NO 
LEFT JOIN (SELECT CARD_NO, SUM(LB) AS LB 
FROM (SELECT CARD_NO, LB, PROMOTION_ID, CREATE_USER 
FROM (SELECT CARD_NO, NVL(SUM(DEVICE_PRICE), 0) AS LB, PROMOTION_ID AS PROMOTION_ID, CREATE_USER 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND UPDATED_DATE > TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO,PROMOTION_ID,CREATE_USER) GROUP  BY CARD_NO, LB, CREATE_USER, PROMOTION_ID 
UNION ALL 
(SELECT CARD_NO,NVL(SUM(DEVICE_PRICE ), 0) AS LB,PROMOTION_ID, CREATE_USER 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND TRANSID IS NULL AND DEVICE_PRICE != 0 GROUP  BY CARD_NO,PROMOTION_ID,CREATE_USER)) GROUP  BY CARD_NO) C ON A.CARD_NO = C.CARD_NO)) WHERE  CARD_NO NOT IN (SELECT CARD_NO 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE DESCR IN ( '2018 оны Найдваргүй авлагаар хаав.','2019 оны Найдваргүй авлагаар хаав.' ))) A 
LEFT JOIN (SELECT CARD_NO,SUM(DEVICE_PRICE) AS AAA 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE < TO_DATE('{0}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 AND UPDATED_DATE < TO_DATE('{0}','YYYYMMDDHH24MISS') AND TRANSID IS NOT NULL GROUP  BY CARD_NO) B ON A.CARD_NO = B.CARD_NO)) WHERE  FB1 != 0 OR INCOME > 0 AND CARD_NO NOT LIKE '100%' AND CARD_NO NOT IN (SELECT CARD_NO FROM   UNI_DISH.BLACKLIST)) AA 
LEFT JOIN (SELECT * FROM UNI_DISH.T_DISH_CUSTOM WHERE  SUBSCRIBER_STATUS = '1') TDC ON AA.CARD_NO = TDC.CARD_NO 
LEFT JOIN UNI_DISH.SYSTEM_YES_USER_TBL SYUT ON TDC.OPERATION_ID = SYUT.USER_NAME 
LEFT JOIN RESELLER.T_BRANCH@UNIDISHDB SBL ON SYUT.BRANCH_ID = SBL.BRANCHID 
LEFT JOIN (SELECT CARD_NO, MAX(PROMOTION_NAME) LEASING_NAME, MAX(CREATE_DATE) AS CREATE_DATE, MAX(CREATE_USER) AS CREATE_USER, MAX(UPDATED_DATE) AS UPDATED_DATE FROM UNI_DISH.D_PRODUCT_PLAN D 
LEFT JOIN UNI_DISH.D_PROMOTION_ID E ON E.ID = D.PROMOTION_ID  WHERE E.PRPD = 1  group by CARD_NO) AAA ON AA.CARD_NO = AAA.CARD_NO) 
UNION ALL 
SELECT CARD_NO,FB1 FIRST_BALANCE,INCOME,CASE WHEN INCOME > 0 AND PP > INCOME THEN INCOME WHEN INCOME > 0 AND PP < INCOME THEN PP ELSE CASE WHEN PP > FB1 AND FB1 > 0 THEN FB1 ELSE PP END END AS AMOUNT, LB LAST_BALANCE, SUBSCRIBER_FNAME, SUBSCRIBER_LNAME,  CERTIFICATE_NO, BRANCH_NAME, CREATE_DATE, UPDATED_DATE, CREATED_USER CREATE_USER, LEASING_NAME 
FROM (SELECT XX.CARD_NO,CASE WHEN INCOME > 0 THEN 0 ELSE FB END AS FB1,INCOME,PP,LB,TDC.SUBSCRIBER_FNAME,TDC.SUBSCRIBER_LNAME, TDC.CERTIFICATE_NO,SBL.BRANCHNAME  BRANCH_NAME,XX.CREATE_DATE,XX.UPDATED_DATE,XX.CREATED_USER,'FLASH LEASING' LEASING_NAME 
FROM (SELECT X.*,CASE WHEN CREATE_DATE >= TO_DATE('{0}','YYYYMMDDHH24MISS') AND CREATE_DATE <= TO_DATE('{1}','YYYYMMDDHH24MISS') THEN FB ELSE 0 END AS INCOME 
FROM (SELECT FL.CARD_NO,CASE 
WHEN FLC.FB IS NULL AND FLCADD.LB IS NOT NULL THEN FLCADD.LB WHEN FLC.FB IS NULL AND FLCADD.LB IS NULL THEN FL.AMOUNT ELSE FLC.FB END AS FB,
NVL(FLC.PP, 0) AS PP,
CASE 
WHEN FLC.LB IS NULL AND FLCADD.LB IS NOT NULL THEN FLCADD.LB WHEN FLC.FB IS NULL AND FLCADD.LB IS NULL THEN FL.AMOUNT ELSE FLC.LB END AS LB, FLC.UPDATED_DATE, CREATE_DATE, CREATED_USER,FL.LIMIT 
FROM UNI_DISH.FLASH_LEASING FL 
LEFT JOIN (SELECT CARD_NO,MAX(F.BEFORE_REMAINS) FB,SUM(F.AMOUNT) PP, MIN(F.AFTER_REMAINS)  LB, MAX(REGDATE) UPDATED_DATE FROM UNI_DISH.FLASH_LEASING_CDR F LEFT JOIN UNI_DISH.ACCOUNT_TRANSACTION T ON T.TRANS_ID = F.TRANS_ID WHERE  REGDATE <= TO_DATE('{1}','YYYYMMDDHH24MISS') AND REGDATE >= TO_DATE('{0}','YYYYMMDDHH24MISS') GROUP  BY CARD_NO) FLC ON FLC.CARD_NO = FL.CARD_NO
LEFT JOIN (SELECT CARD_NO,MIN(F.AFTER_REMAINS) LB, MAX(REGDATE) UPDATED_DATE FROM UNI_DISH.FLASH_LEASING_CDR F LEFT JOIN UNI_DISH.ACCOUNT_TRANSACTION T ON T.TRANS_ID = F.TRANS_ID WHERE  REGDATE <= TO_DATE('{0}','YYYYMMDDHH24MISS') GROUP  BY CARD_NO) FLCADD ON FLCADD.CARD_NO = FL.CARD_NO
WHERE SALED_DATE<=TO_DATE('{1}','YYYYMMDDHH24MISS')
) X) XX 
LEFT JOIN (SELECT * FROM   UNI_DISH.T_DISH_CUSTOM WHERE  SUBSCRIBER_STATUS = '1') TDC ON XX.CARD_NO = TDC.CARD_NO 
LEFT JOIN UNI_DISH.SYSTEM_YES_USER_TBL SYUT ON TDC.OPERATION_ID = SYUT.USER_NAME
LEFT JOIN RESELLER.T_BRANCH@UNIDISHDB SBL ON SYUT.BRANCH_ID = SBL.BRANCHID
)
) A 
...
WHERE  CARD_NO IN (SELECT CARD_NO FROM   UNI_DISH.T_DISH_CUSTOM WHERE  SUBSCRIBER_STATUS = '1') AND CARD_NO NOT IN (SELECT CARD_NO FROM UNI_DISH.TEST_CARDS) AND CARD_NO NOT LIKE '100%' AND LAST_BALANCE >= 0 AND LEASING_NAME IS NOT NULL           

UNION ALL

SELECT CARD_NO,FIRST_BALANCE,INCOME,AMOUNT, LAST_BALANCE,SUBSCRIBER_FNAME,SUBSCRIBER_LNAME,CERTIFICATE_NO, 
    CASE WHEN BRANCH IS NULL THEN CREATE_USER ELSE BRANCH END AS BRANCH_NEW, CREATE_DATE, UPDATED_DATE, CREATE_USER, LEASING_NAME 
    FROM (SELECT A.*,SBL.BRANCH_NAME BRANCH 
    FROM (SELECT CARD_NO,FIRST_BALANCE,INCOME,AMOUNT,LAST_BALANCE,SUBSCRIBER_FNAME,SUBSCRIBER_LNAME,CERTIFICATE_NO,BRANCH_NAME,CREATE_DATE,UPDATED_DATE,CREATE_USER,LEASING_NAME 
    FROM (SELECT AA.CARD_NO,FIRST_BALANCE,INCOME,AMOUNT,LAST_BALANCE, TDC.SUBSCRIBER_FNAME, TDC.SUBSCRIBER_LNAME,TDC.CERTIFICATE_NO,AAA.CREATE_DATE, AAA.UPDATED_DATE, AAA.CREATE_USER, AAA.LEASING_NAME, SBL.BRANCHNAME BRANCH_NAME 
    ...
    FROM (SELECT CARD_NO,FB,INCOME,PP,LB 
    FROM (SELECT A.CARD_NO,FB,INCOME,PP, LB 
    FROM (SELECT CARD_NO,SUM(DEVICE_PRICE) FB, 0 AS INCOME, AVG(DEVICE_PRICE) DEVICE_PRICE 
    FROM UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE('{0}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 AND PROMOTION_ID IN (SELECT ID FROM UNI_DISH.D_PROMOTION_ID WHERE PRPD=2)  GROUP  BY CARD_NO, 0) A 
    LEFT JOIN (SELECT CARD_NO , SUM(DEVICE_PRICE) AS PP 
    FROM UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE('{0}' , 'YYYYMMDDHH24MISS' ) AND UPDATED_DATE BETWEEN TO_DATE('{0}','YYYYMMDDHH24MISS')  AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO) B ON A.CARD_NO = B.CARD_NO 
    LEFT JOIN (SELECT CARD_NO, SUM(LB) AS LB 
    FROM (SELECT CARD_NO, LB 
    FROM (SELECT CARD_NO, NVL(SUM(DEVICE_PRICE), 0) AS LB 
    FROM UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE('{0}' , 'YYYYMMDDHH24MISS') AND UPDATED_DATE NOT BETWEEN TO_DATE( '{0}', 'YYYYMMDDHH24MISS') AND TO_DATE('{1}','YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO) 
UNION ALL 
(SELECT CARD_NO, NVL(SUM(DEVICE_PRICE), 0 ) AS LB 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE CREATE_DATE < TO_DATE( '{0}' , 'YYYYMMDDHH24MISS') AND TRANSID IS NULL AND DEVICE_PRICE != 0 GROUP  BY CARD_NO)) GROUP  BY CARD_NO) C ON A.CARD_NO = C.CARD_NO) 
...
(SELECT A.CARD_NO, FB, INCOME, PP, LB 
FROM (SELECT CARD_NO, 0 AS FB, SUM(DEVICE_PRICE) INCOME, PROMOTION_ID, CREATE_USER 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE( '{0}', 'YYYYMMDDHH24MISS' ) AND TO_DATE('{1}', 'YYYYMMDDHH24MISS' ) AND DEVICE_PRICE != 0 AND PROMOTION_ID IN (SELECT ID FROM UNI_DISH.D_PROMOTION_ID WHERE PRPD=2) GROUP  BY CARD_NO, 0, CREATE_USER, PROMOTION_ID) A 
LEFT JOIN (SELECT CARD_NO, NVL(SUM(DEVICE_PRICE) , 0) AS PP, PROMOTION_ID, CREATE_USER 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE( '{0}', 'YYYYMMDDHH24MISS') AND TO_DATE( '{1}', 'YYYYMMDDHH24MISS' ) AND UPDATED_DATE BETWEEN TO_DATE( '{0}', 'YYYYMMDDHH24MISS') AND TO_DATE('{1}', 'YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 GROUP  BY CARD_NO, PROMOTION_ID, CREATE_USER) B ON A.CARD_NO = B.CARD_NO 
LEFT JOIN (SELECT CARD_NO, SUM(LB) AS LB 
FROM (SELECT CARD_NO, LB, PROMOTION_ID, CREATE_USER 
...
UNION ALL 
(SELECT CARD_NO, NVL(SUM(DEVICE_PRICE ), 0) AS LB, PROMOTION_ID, CREATE_USER 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE  CREATE_DATE BETWEEN TO_DATE( '{0}' , 'YYYYMMDDHH24MISS') AND TO_DATE('{1}', 'YYYYMMDDHH24MISS' )  AND TRANSID IS NULL AND DEVICE_PRICE != 0 GROUP  BY CARD_NO, PROMOTION_ID, CREATE_USER)) GROUP  BY CARD_NO) C ON A.CARD_NO = C.CARD_NO)) WHERE  CARD_NO NOT IN (SELECT CARD_NO FROM   UNI_DISH.D_PRODUCT_PLAN WHERE DESCR IN ( '2018 оны Найдваргүй авлагаар хаав.' , '2019 оны Найдваргүй авлагаар хаав.' ))) A 
LEFT JOIN (SELECT CARD_NO, SUM(DEVICE_PRICE) AS AAA 
FROM UNI_DISH.D_PRODUCT_PLAN WHERE PROMOTION_ID IN (SELECT ID FROM UNI_DISH.D_PROMOTION_ID WHERE PRPD=2) AND CREATE_DATE < TO_DATE('{0}', 'YYYYMMDDHH24MISS') AND DEVICE_PRICE != 0 AND UPDATED_DATE < TO_DATE('{0}', 'YYYYMMDDHH24MISS') AND TRANSID IS NOT NULL GROUP  BY CARD_NO) B ON A.CARD_NO = B.CARD_NO)) WHERE  FB1 != 0 OR INCOME > 0 AND CARD_NO NOT LIKE '100%' AND CARD_NO NOT IN (SELECT CARD_NO 
FROM UNI_DISH.BLACKLIST))
AA 
LEFT JOIN (SELECT * 
FROM UNI_DISH.T_DISH_CUSTOM WHERE  SUBSCRIBER_STATUS = '1') TDC ON AA.CARD_NO = TDC.CARD_NO 
LEFT JOIN UNI_DISH.SYSTEM_YES_USER_TBL SYUT ON TDC.OPERATION_ID = SYUT.USER_NAME 
LEFT JOIN RESELLER.T_BRANCH@UNIDISHDB SBL ON SYUT.BRANCH_ID = SBL.BRANCHID 
LEFT JOIN (SELECT CARD_NO, MAX(PROMOTION_NAME) LEASING_NAME, MAX(CREATE_DATE)    AS CREATE_DATE, MAX(CREATE_USER)    AS CREATE_USER, MAX(UPDATED_DATE)   AS UPDATED_DATE 
FROM UNI_DISH.D_PRODUCT_PLAN D 
LEFT JOIN UNI_DISH.D_PROMOTION_ID E ON E.ID = D.PROMOTION_ID  WHERE E.PRPD = 2  group by CARD_NO ) AAA ON AA.CARD_NO = AAA.CARD_NO) 
UNION ALL 
SELECT CARD_NO, FB1 FIRST_BALANCE, INCOME, CASE WHEN INCOME > 0 AND PP > INCOME THEN INCOME WHEN INCOME > 0 AND PP < INCOME THEN PP ELSE CASE WHEN PP > FB1 AND FB1 > 0 THEN FB1 ELSE PP END END AS AMOUNT, LB LAST_BALANCE, SUBSCRIBER_FNAME, SUBSCRIBER_LNAME, CERTIFICATE_NO, BRANCH_NAME, CREATE_DATE, UPDATED_DATE, CREATED_USER CREATE_USER, LEASING_NAME 
FROM (SELECT XX.CARD_NO,CASE WHEN INCOME > 0 THEN 0 ELSE FB END AS FB1,INCOME,PP,LB,TDC.SUBSCRIBER_FNAME,TDC.SUBSCRIBER_LNAME, TDC.CERTIFICATE_NO,SBL.BRANCHNAME  BRANCH_NAME,XX.CREATE_DATE,XX.UPDATED_DATE,XX.CREATED_USER,'FLASH LEASING' LEASING_NAME 
FROM (SELECT X.*,CASE WHEN CREATE_DATE >= TO_DATE('{0}','YYYYMMDDHH24MISS') AND CREATE_DATE <= TO_DATE('{1}','YYYYMMDDHH24MISS') THEN FB ELSE 0 END AS INCOME 
...
WHERE A.CARD_NO NOT LIKE '100%' AND A.CREATE_DATE>=TO_DATE('{0}','YYYYMMDDHH24MISS') AND  A.CREATE_DATE<=TO_DATE('{1}','YYYYMMDDHH24MISS') AND A.TRANS_ID NOT IN (SELECT TRANS_ID FROM FLASH_LEASING_CDR)", begin,end);
        ...
        public static string rpt26(string begin, string end)
        {
            string qry = string.Format(@"SELECT CREATE_USER AS INSTALLER,
                ...
                COUNT(CASE WHEN PURCHASE_TYPE = 'Telesales' THEN 1 END) AS TELESALES ,
                COUNT(CASE WHEN PURCHASE_TYPE IN ('Үйлчилгээний хөлс (А бүс)','Үйлчилгээний хөлс (Б бүс)') THEN 1 END) - COUNT(CASE WHEN PURCHASE_TYPE = 'CASH' THEN 1 END) AS SERVICE ,
                COUNT(CASE WHEN PURCHASE_TYPE = 'Үйлчилгээний хөлс (Салаалах)' THEN 1 END) AS SALAALAH ,
                COUNT(CASE WHEN PURCHASE_TYPE = 'Үйлчилгээний хөлс (Б бүс)' THEN 1 END) AS B_BUS ,
                NVL(SUM(CASE WHEN PURCHASE_TYPE = 'Payment' THEN BUNDLE_PRICE END),0) AS PAYMENT ,
                COUNT(CASE WHEN PURCHASE_TYPE = 'TCA' THEN 1 END) AS TCA FROM ( SELECT * FROM (
                ...
                WHEN BUNDLE_PRICE IN (230000,300000) AND BUNDLE_ID IN (SELECT ID FROM W_BUNDLE WHERE SERVICE_TYPE = 1) THEN 'CASH' END AS PURCHASE_TYPE
                ...
                UNION ALL
                SELECT CARD_NUMBER , AMOUNT , USER_NAME ,TRANS_ID , REGDATE ,DESCR ,DESCR AS PURCHASE_TYPE
                ...
                UNION ALL
                SELECT CARD_NO, AMOUNT,INSTALLER ,TRANS_ID , INSERT_DATE,'Payment' AS PURCHASE_TYPE ,'Payment' AS TYPE2 FROM UNI_DISH.INSTALLER_PROMOTION_TBL_NEW A
