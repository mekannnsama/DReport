...
WHERE A.CARD_NO NOT LIKE '100%' AND A.CREATE_DATE>=TO_DATE('{0}','YYYYMMDDHH24MISS') AND  A.CREATE_DATE<=TO_DATE('{1}','YYYYMMDDHH24MISS') AND A.TRANS_ID NOT IN (SELECT TRANS_ID FROM FLASH_LEASING_CDR)", begin, end);
        ...
        public static string rpt26(string begin, string end)
        {
            string qry = string.Format(@"SELECT CREATE_USER AS INSTALLER,
                ...
                SUM(CASE WHEN PURCHASE_TYPE IN ('Үйлчилгээний хөлс (А бүс)') THEN AMOUNT/15000 WHEN PURCHASE_TYPE = 'Үйлчилгээний хөлс (Б бүс)' THEN AMOUNT/20000 END) - COUNT(CASE WHEN PURCHASE_TYPE = 'CASH' THEN 1 END) AS SERVICE ,
                SUM(CASE WHEN PURCHASE_TYPE = 'Үйлчилгээний хөлс (Салаалах)' THEN AMOUNT/8000 END) AS SALAALAH ,
                SUM(CASE WHEN PURCHASE_TYPE = 'Үйлчилгээний хөлс (Б бүс)' THEN AMOUNT/20000 END) AS B_BUS ,
